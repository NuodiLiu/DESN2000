...
| UC-09 Send secure in-app messages | UC-11 View verified housemate profiles | Extend / Optional | Core secure messaging (personal contact details hidden) works on its own; UC-11 adds the verification flag that marks unverified counterparties for extra trust. |
...
| UC-12 Filter housemates by lifestyle preferences | UC-11 View verified housemate profiles | Required | Compatibility matching is only offered between verified profiles to preserve trust; a match does not grant household access. |
