# User Story Prototype Screenshots

RoomMate Haven prototype evidence for the 18 stable user story IDs in
[`part2/userstories.md`](../userstories.md).

Each PNG is 1200 x 900 and shows the website state that best demonstrates the
corresponding user story.

| User story | Prototype evidence | Screenshot |
| --- | --- | --- |
| US-01 | Rotating chore roster | `US-01_rotating-chore-roster.png` |
| US-02 | Written household rules | `US-02_household-rules.png` |
| US-03 | Chore system proposal and approval | `US-03_chore-system-proposal.png` |
| US-04 | Log a shared expense | `US-04_log-shared-expense.png` |
| US-05 | Outstanding household balances | `US-05_outstanding-balances.png` |
| US-06 | Split a utility bill | `US-06_split-utility-bill.png` |
| US-07 | Household noticeboard | `US-07_household-noticeboard.png` |
| US-08 | Anonymous household issue | `US-08_anonymous-household-issue.png` |
| US-09 | Secure in-app messaging | `US-09_secure-in-app-messaging.png` |
| US-10 | Unanimous house-rule approval and activation | `US-10_agreed-noise-rules.png` |
| US-11 | Verified housemate profiles | `US-11_verified-housemate-profiles.png` |
| US-12 | Profile-derived compatibility ranking | `US-12_lifestyle-housemate-filters.png` |
| US-13 | Multi-criteria property and affordability filters | `US-13_property-search-filters.png` |
| US-14 | Overdue chore and expense reminders | `US-14_overdue-reminders.png` |
| US-15 | Verified former-housemate reviews | `US-15_housemate-reviews.png` |
| US-16 | Rent tracking and reminders | `US-16_rent-tracking-reminders.png` |
| US-17 | Publish a property listing | `US-17_publish-property-listing.png` |
| US-18 | Tenant announcements | `US-18_tenant-announcements.png` |
