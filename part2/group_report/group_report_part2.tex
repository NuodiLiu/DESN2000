    ...
    \fcolorbox{midgray}{softgray}{%
        \begin{minipage}{0.92\textwidth}
            ...
            \begin{tabularx}{\textwidth}{@{}L{2.0cm}Y Y Y@{}}
                ...
                Pranav Shanker & 4, 4, 5, 4, 3, 4 & 4, 4, 4, 5, 4, 4 & 5, 5, 3, 5, 5, 3\\
                \midrule
...
Table~\ref{tabstorymap} summarises the release slice used to define Sprint~1. The complete team map, including personas, all four activities, backbone tasks and release priorities, is provided in Figure~\ref{figfullstorymap} in Appendix~\ref{appusecasemapping}.
...
This fidelity is appropriate because the rubric requires observable, testable interactions linked to user stories. A clickable interface allows the team to measure completion, errors and comprehension, while avoiding the cost and privacy risk of a production backend at this stage. The prototype does not perform real identity checks, production encryption, database persistence or external translation services. Property filtering, compatibility calculation and unanimous rule approval run locally against seeded data, while the phone check uses a demonstration code. The prototype therefore tests the interaction logic and user comprehension without claiming production security or independently authenticated housemate accounts.
...
The screens demonstrate both the normal path and observable system feedback: multi-criteria property filters update and sort the result set, lifestyle preferences recalculate compatibility scores, contradictory schedules are rejected, messaging remains locked until verification, personal contact details trigger a warning, language changes update rule content, and a proposed rule remains inactive until every housemate approves it.

\section{Technical Development Evolution}
\label{secdevevolution}

During Weeks~3 to~7, the proposed solution gradually evolved as new research findings and team discussions shaped the design. Initially, the project focused on creating a general student accommodation platform where users could browse available properties and contact landlords. However, the user research indicated that students were more concerned about trust and housemate compatibility than simply finding available rooms.

Based on these findings, the team shifted the design towards a compatibility-focused platform. Lifestyle preference profiles and compatibility comparison became the core feature, supported by verified listings and guided in-app messaging to improve trust during communication. Finally, a lightweight co-living agreement was added to encourage discussion of household expectations before students moved in.

This iterative process ensured that the final concept directly addressed the problems identified during the Empathise and Define stages while remaining achievable within the project scope. The resulting Trust-to-Agreement journey described in Section~\ref{secconceptselection} reflects this progression from a broad accommodation search tool to a student-centred decision path.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[evostep] (w3) {\textbf{Week 3}\\Initial idea: general accommodation search platform};
        \node[evostep, below=of w3] (w4) {\textbf{Week 4}\\Research findings: students care about trust and housemate compatibility};
        \node[evostep, below=of w4] (w5) {\textbf{Week 5}\\Concept selection: added lifestyle compatibility matching};
        \node[evostep, below=of w5] (w6) {\textbf{Week 6}\\System design: added verified listings and guided messaging};
        \node[evostep, below=of w6] (w7) {\textbf{Week 7}\\Final prototype: integrated compatibility matching, trust indicators and co-living agreement};
...
Each screen is mapped to the user story and acceptance criteria that it demonstrates. Table~\ref{tabprototypetraceability} identifies the exact interaction evidence and keeps production-system boundaries separate from prototype task completion. The same screen identifiers are used by the test plan in Section~\ref{sectestplan}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}L{0.9cm}L{2.4cm}L{1.4cm}L{2.5cm}Y L{1.1cm}@{}}
        ...
        \midrule
        S-01 & Multi-criteria property search & US-13 & AC-US13-01--04, AC-US13-06 & Combine university or suburb, budget, room type, distance, bedrooms and move-in date; results update, sort by campus distance, show affordability and provide a broadening suggestion when no exact match exists. & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/\#discover/properties}{Open}\\
        ...
        S-03 & Profile-derived compatibility ranking & US-12 & AC-US12-01, AC-US12-02, AC-US12-04--06 & Edit the lifestyle profile to recalculate and re-rank scores, inspect the explanation, submit incomplete or conflicting preferences for validation, and broaden strict filters after a zero-result state. & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/\#discover/housemates}{Open}\\
        ...
        S-06 & Rule proposal and unanimous approval & US-10, US-02 & AC-US10-01, AC-US10-02, AC-US10-04, AC-US02-01 & Submit an incomplete rule for validation, send a complete proposal to every housemate, approve or request changes as each reviewer, and confirm the rule enters the active list with an agreement timestamp only after all approvals. & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/\#household/rules}{Open}\\
        \bottomrule
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}L{0.9cm}Y L{1.9cm}L{3.6cm}L{1.9cm}@{}}
        ...
        T3 & Build a profile and use recalculated compatibility & US-12 & AC-US12-01, AC-US12-02, AC-US12-04--06 & Liam\\
        T4 & Complete authentication and message safely & US-09 & AC-US09-01, AC-US09-04 & Qian Cao\\
        T5 & Propose, review and activate a house rule & US-02, US-10 & AC-US02-01--03, AC-US10-01, AC-US10-02, AC-US10-04 & Liam\\
        \bottomrule
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}L{3.4cm}Y@{}}
        ...
        Task scenario & ``Save a very tidy, mostly quiet profile with occasional guests and a flexible schedule. Identify the best match and explain the score. Then select both Early riser and Late sleeper, and finally apply all three strict lifestyle filters.''\\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}L{3.4cm}Y@{}}
        ...
        Task scenario & ``Open the quiet-hours explanation, then propose a new late-gathering rule. First submit it without a category, then complete it. Record one change request, revise it, and approve as Emily, Daniel and Mia.''\\
...
Figure~\ref{figfullstorymap} presents the complete User Story Map used to organise personas, product activities, backbone tasks and release priorities. It provides the visual planning context for the detailed traceability that follows, where each stable story identifier maps to one goal-level use case in Table~\ref{tabusucmapping}.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{user-story-map.png}
    \caption{Complete User Story Map showing personas, activities, backbone tasks and release priorities.}
    \label{figfullstorymap}
\end{figure}
\end{landscape}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}L{0.9cm}L{0.9cm}L{2.4cm}L{2.2cm}Y L{1.1cm}L{1.7cm}@{}}
        ...
        \midrule
        US-01 & EP-02 & UC-01 Set up and manage chore roster & Domestic student / shared-house resident & The user can create, assign and rotate chores among housemates and track completion history. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-01}{View}\\
        US-02 & EP-02 & UC-02 View and manage household rules & International student / shared-house resident & The user can view all household rules and expectations in their preferred language in a single location. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-02}{View}\\
        US-03 & EP-02 & UC-03 Propose and approve chore system & Domestic student & All housemates have reviewed and approved a structured chore system before it becomes active. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-03}{View}\\
        US-04 & EP-02 & UC-04 Log and split shared expenses & Shared-house resident & The user can log a shared expense and the system automatically calculates and assigns each housemate's share. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-04}{View}\\
        US-05 & EP-02 & UC-05 Track outstanding balances & International student / shared-house resident & The user can view a real-time summary of who owes what and mark balances as settled. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-05}{View}\\
        US-06 & EP-02 & UC-06 Split utility bills & International student / shared-house resident & The user can input a utility bill and split it equally or by usage among selected housemates. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-06}{View}\\
        US-07 & EP-02 & UC-07 Post and view household notices & Shared-house resident & The user can post a notice visible to all housemates on a dedicated noticeboard separate from general messaging. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-07}{View}\\
        US-08 & EP-02 & UC-08 Raise anonymous household issues & Domestic student / shared-house resident & A household issue is posted to the shared space without revealing the submitter's identity if the anonymous option is selected. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-08}{View}\\
        US-09 & EP-01 & UC-09 Send secure in-app messages & International student / shared-house resident & The user can send and receive messages without either party's personal contact details being visible. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-09}{View}\\
        US-10 & EP-02 & UC-10 Set and view agreed house rules & Shared-house resident & All housemates have acknowledged a proposed house rule before it becomes active and can reference it during disputes. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-10}{View}\\
        US-11 & EP-01 & UC-11 View verified housemate profiles & International student / accommodation seeker & The user can view verified profiles with identity badges and filter search results to show only verified profiles. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-11}{View}\\
        US-12 & EP-01 & UC-12 Filter housemates by lifestyle preferences & Shared-house resident / accommodation seeker & The user can set lifestyle preference filters and view a compatibility score and summary for each potential housemate. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-12}{View}\\
        US-13 & EP-01 & UC-13 Search and filter property listings & Domestic student / accommodation seeker & The user can search for listings by university proximity and apply filters for rent, distance, bedrooms and move-in date. & MVP & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-13}{View}\\
        US-14 & EP-03 & UC-14 Receive overdue chore and expense reminders & Shared-house resident & The user receives automatic in-app reminders when a chore or expense payment is overdue. & Extension & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-14}{View}\\
        US-15 & EP-03 & UC-15 Read and submit housemate reviews & Shared-house resident / accommodation seeker & The user can read reviews left by previous housemates and submit a review after a confirmed tenancy ends. & Extension & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-15}{View}\\
        US-16 & EP-04 & UC-16 Track rent and send reminders & Property manager & Rent status is recorded per tenant and overdue tenants receive automatic reminders. & Secondary & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-16}{View}\\
        US-17 & EP-04 & UC-17 Create and manage property listings & Property manager & A complete property listing is published and visible to students searching for accommodation. & Secondary & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-17}{View}\\
        US-18 & EP-04 & UC-18 Send tenant announcements & Property manager & An announcement is delivered to all tenants of a selected property with read receipt tracking. & Secondary & \href{http://roommate-haven-dev.eba-2h6p4mxq.ap-southeast-2.elasticbeanstalk.com/?capture=US-18}{View}\\
        \bottomrule
    \end{tabularx}
